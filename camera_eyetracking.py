...
#############################################################################################################
    ...
    def camera(self):
        ...
        for i in self.link_index:
            ...
            # local_forward = np.array([0, 1, 0]) 
            # world_forward = rot_matrix@local_forward  
            # target_position = position+world_forward 
            ...
            img_data =p.getCameraImage(width, height, view_matrix, projection_matrix)


            rgb_img =np.reshape(img_data[2], (height, width, 4))[:, :, :3] 
...
######################################################################
    ...
    def step(self, action=None, num_steps=5):
        """
